...
fn main() -> Result<(), Box<dyn Error>> {
  ...
  match opts.subcmd {
    SubCommand::Build { mut path } => build(&mut path).map(drop),
    SubCommand::Run { mut path } => {
      let wasm = build(&mut path)?;
      run(wasm)
    }
  }
}

fn build(path: &mut PathBuf) -> Result<Vec<u8>, Box<dyn Error>> {
  let input = fs::read_to_string(&path)?;
  println!("------------------ Sycamore Input ------------------");
  println!("{}", input);
  let parsed = SycParser::new(&input).parse();
  let wasm = Codegen::new(parsed).generate();
...
fn run(wasm: Vec<u8>) -> Result<(), Box<dyn Error>> {
  ...
  let print = Func::wrap(
    &mut store,
    |mut caller: Caller<'_, u32>, offset: i32, len: i32| {
      let data = &caller
